...
import 'package:wave_mall_user/features/tracking/widget/line_dashed_painter.dart';
...
class _TrackingResultScreenState extends State<TrackingResultScreen> {
  ...
  void initState() {
    Provider.of<OrderProvider>(context, listen: false)
        .initTrackingInfo(widget.orderID);
    super.initState();
  }
                ...
                return tracking.trackingModel != null
                    ? ListView(
                        children: [
                          ...
                          CustomStepper(
                              title:
                                  '${getTranslated('order_confirmed', context)}',
                              icon: Images.orderConfirmedIcon,
                              checked: (status == statusList[1] ||
                                  status == statusList[2] ||
                                  status == statusList[3] ||
                                  status == statusList[4])),
                          CustomStepper(
                              ...
                              title:
                                  '${getTranslated('preparing_for_shipment', context)}',
                              checked: (status == statusList[2] ||
                                  status == statusList[3] ||
                                  status == statusList[4])),
                          CustomStepper(
...
class CustomStepper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Color myColor;
    if (checked) {
      myColor = Provider.of<ThemeProvider>(context, listen: false).darkTheme
          ? Colors.white
            ...
            child: Column(
              ...
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.start, children: [
                  Padding(
                      ...
                      child: Container(
                        ...
                        child: SizedBox(
                            height: 30,
                            child: Image.asset(icon, color: myColor)),
                      )),
                  Column(
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              SizedBox(
                                  height: 20,
                                  child: Image.asset(Images.dateTimeIcon)),
                              const SizedBox(
                                  width: Dimensions.paddingSizeExtraSmall),
                              Text(
                                  DateConverter.dateTimeStringToDateAndTime(
                                      dateTime!),
                ...
                isLastItem
                    ...
                    : Padding(
                        padding: const EdgeInsets.only(
                            top: Dimensions.paddingSizeExtraSmall, left: 35),
                        child:
                            CustomPaint(painter: LineDashedPainter(myColor))),
              ],
            ),
          ),
          if (checked)
