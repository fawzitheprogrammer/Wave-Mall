...
class CouponItem extends StatelessWidget {
  ...
  const CouponItem(
      {super.key, required this.coupons, this.fromCheckout = false});

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: fromCheckout
          ? const EdgeInsets.fromLTRB(0, Dimensions.paddingSizeSmall, 0, 0)
          : const EdgeInsets.fromLTRB(Dimensions.paddingSizeDefault,
              Dimensions.paddingSizeSmall, Dimensions.fontSizeDefault, 0),
      child: Stack(
                        ...
                        child: Column(
                          children: [
                            SizedBox(
                                width: 30,
                                child: Image.asset(
                                    color: coupons.discountType == 'percentage'
                            ...
                            coupons.couponType == 'free_delivery'
                                ...
                                : coupons.discountType == 'percentage'
                                    ? Padding(
                                        ...
                                        child: Text(
                                          '${coupons.discount} ${'% ${getTranslated('off', context)}'}',
                                          style: robotoBold.copyWith(
                                              fontSize:
                            ...
                            Text(
                                coupons.seller != null
                                    ? coupons.seller?.shop?.name ?? ''
                                    : coupons.sellerId == 0
                                        ? '${getTranslated('on_all_shop', context)}'
                          ...
                          children: [
                            ...
                            Padding(
                                padding: const EdgeInsets.symmetric(
                                    vertical: Dimensions.paddingSizeSmall),
                                child: Text(
                                    '${getTranslated('valid_till', context)} ${DateConverter.estimatedDateYear(DateTime.parse(coupons.planExpireDate!))}',
                                    style: textRegular.copyWith())),
                            Padding(
                              padding: const EdgeInsets.only(
                                  bottom: Dimensions.paddingSizeSmall),
                              child: Text(
                                  '${getTranslated('available_from', context)} ${PriceConverter.convertPrice(context, coupons.minPurchase)}',
              ...
              child: InkWell(
                  onTap: () async {
                    await Clipboard.setData(
                        ClipboardData(text: coupons.code ?? ''));
                    showCustomSnackBar(
                        getTranslated(
                            'coupon_code_copied_successfully', Get.context!),
                        Get.context!,
                        isError: false);
                  },
                  child: Padding(
