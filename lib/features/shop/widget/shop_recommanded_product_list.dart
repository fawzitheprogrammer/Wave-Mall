...
class ShopRecommandedProductViewList extends StatefulWidget {
  ...
  final int sellerId;
  const ShopRecommandedProductViewList(
      {super.key, required this.scrollController, required this.sellerId});

  @override
  State<ShopRecommandedProductViewList> createState() =>
      _ShopRecommandedProductViewListState();
}

class _ShopRecommandedProductViewListState
    extends State<ShopRecommandedProductViewList> {
  @override
  Widget build(BuildContext context) {
    return Consumer<ProductProvider>(builder: (context, productProvider, _) {
      return productProvider.sellerWiseRecommandedProduct != null
          ? (productProvider.sellerWiseRecommandedProduct != null &&
                  productProvider.sellerWiseRecommandedProduct!.products !=
                      null &&
                  productProvider
              ...
              ? PaginatedListView(
                  ...
                  onPaginate: (offset) async {
                    await productProvider.getSellerProductList(
                        widget.sellerId.toString(), offset!, context);
                  },
                  totalSize:
                      productProvider.sellerWiseRecommandedProduct?.totalSize,
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return ProductWidget(
                          productModel: productProvider
                              .sellerWiseRecommandedProduct!.products![index]);
                    },
                  ))
              : const SizedBox()
          : ProductShimmer(
              isEnabled: productProvider.sellerWiseRecommandedProduct == null,
              isHomePage: false);
    });
  }
}
