...
class CartRepository {
  ...
  Future<ApiResponse> getCartListData() async {
    try {
      final response = await dioClient!.get(
          '${AppConstants.getCartDataUri}?guest_id=${Provider.of<AuthController>(Get.context!, listen: false).getGuestToken()}');
  ...
  Future<ApiResponse> addToCartListData(CartModelBody cart,
  ...
  Future<ApiResponse> updateQuantity(int? key, int quantity) async {
    try {
      final response =
          await dioClient!.post(AppConstants.updateCartQuantityUri, data: {
        '_method': 'put',
        'key': key,
        'quantity': quantity,
        'guest_id': Provider.of<AuthController>(Get.context!, listen: false)
  ...
  Future<ApiResponse> removeFromCart(int? key) async {
    try {
      final response =
          await dioClient!.post(AppConstants.removeFromCartUri, data: {
        '_method': 'delete',
        'guest_id': Provider.of<AuthController>(Get.context!, listen: false)
            .getGuestToken(),
        'key': key
  ...
  Future<ApiResponse> getSelectedShippingType(
    ...
    try {
      final response = await dioClient!.get(
          '${AppConstants.getSelectedShippingTypeUri}?seller_id=$sellerId&seller_is=$sellerType');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
