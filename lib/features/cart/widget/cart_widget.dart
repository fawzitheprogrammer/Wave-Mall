                  ...
                  SlidableAction(
                      onPressed: (value) {
                        Provider.of<CartController>(context, listen: false)
                            .removeFromCartAPI(context, cartModel!.id, index);
                      },
                      backgroundColor:
...
class QuantityButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Consumer<CartController>(builder: (context, cartProvide, _) {
      return InkWell(
        onTap: () {
          if (!isIncrement && quantity! > minimumOrderQuantity!) {
            Provider.of<CartController>(context, listen: false)
                .updateCartProductQuantity(cartModel!.id,
                    cartModel!.quantity! - 1, context, false, index)
                .then((value) {
            ...
            Provider.of<CartController>(context, listen: false)
                .updateCartProductQuantity(cartModel!.id,
                    cartModel!.quantity! + 1, context, true, index)
                .then((value) {
              showCustomSnackBar(value.message, context,
                  isError: !value.isSuccess);
            });
