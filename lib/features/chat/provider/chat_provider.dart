...
import 'package:flutter_sixvalley_ecommerce/features/chat/domain/repo/chat_repo.dart';
...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> getChatList(BuildContext context, int offset,
    ...
    ApiResponse apiResponse = await chatRepo!
        .getChatList(_userTypeIndex == 0 ? 'seller' : 'delivery-man', offset);
  ...
  Future<void> searchChat(BuildContext context, String search) async {
    ...
    notifyListeners();
    ApiResponse apiResponse = await chatRepo!
        .searchChat(_userTypeIndex == 0 ? 'seller' : 'delivery-man', search);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      chatModel = null;
      chatModel = ChatModel(totalSize: 1, limit: '10', offset: '1', chat: []);
      apiResponse.response!.data
          .forEach((chat) => chatModel!.chat!.add(Chat.fromJson(chat)));
  ...
  Future<http.StreamedResponse> sendMessage(MessageBody messageBody) async {
    _isMessageSend = true;
    notifyListeners();
    http.StreamedResponse response = await chatRepo!.sendMessage(
      messageBody,
      _userTypeIndex == 0 ? 'seller' : 'delivery-man',
    ...
    if (response.statusCode == 200) {
      getMessageList(Get.context!, messageBody.id, 1, reload: true);
      _pickedImageFiles = [];
      pickedImageFileStored = [];
  ...
  Future<ApiResponse> seenMessage(
    ...
    ApiResponse apiResponse = await chatRepo!.seenMessage(
