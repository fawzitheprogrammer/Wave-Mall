      ...
      if (token != null) {
        ...
        if (Provider.of<SplashProvider>(context, listen: false)
          ...
          Provider.of<AuthController>(context, listen: false)
              .sendOtpToEmail(socialLogin.email.toString(), temporaryToken)
              .then((value) async {
          ...
          child: Row(
            ...
            children: [
              Provider.of<SplashProvider>(context, listen: false)
                  ...
                  ? InkWell(
                      onTap: () async {
                        ...
                        if (context.mounted) {}
                        if (Provider.of<GoogleSignInController>(Get.context!,
                                    listen: false)
                                .googleAccount !=
                            null) {
                          id = Provider.of<GoogleSignInController>(Get.context!,
                                  listen: false)
                              .googleAccount!
                              .id;
                          email = Provider.of<GoogleSignInController>(
                                  Get.context!,
                                  listen: false)
                              .googleAccount!
                              .email;
                          token = Provider.of<GoogleSignInController>(
                                  Get.context!,
                                  listen: false)
                              .auth
                              .accessToken;
                          medium = 'google';
                          log('eemail =>$email token =>$token =>$id');

                          socialLogin.email = email;
                          socialLogin.medium = medium;
                          socialLogin.token = token;
                          socialLogin.uniqueId = id;
                          await Provider.of<AuthController>(Get.context!,
                                  listen: false)
              ...
              Provider.of<SplashProvider>(context, listen: false)
                  ...
                  ? InkWell(
                      onTap: () async {
                        await Provider.of<FacebookLoginController>(context,
                                listen: false)
                            .login();
                        String? id, token, email, medium;
                        if (Provider.of<FacebookLoginController>(Get.context!,
                                    listen: false)
                                .userData !=
                            null) {
                          id = Provider.of<FacebookLoginController>(
                                  Get.context!,
                                  listen: false)
                              .result
                              .accessToken!
                              .userId;
                          email = Provider.of<FacebookLoginController>(
                                  Get.context!,
                                  listen: false)
                              .userData!['email'];
                          token = Provider.of<FacebookLoginController>(
                                  Get.context!,
                                  listen: false)
                              .result
                              .accessToken!
