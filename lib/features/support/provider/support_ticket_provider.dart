import 'package:wave_mall_user/features/support/domain/model/support_reply_model.dart';
import 'package:wave_mall_user/helper/api_checker.dart';
import 'package:flutter/material.dart';
import 'package:wave_mall_user/features/support/domain/model/support_ticket_body.dart';
...
class SupportTicketProvider extends ChangeNotifier {
  ...
  Future<http.StreamedResponse> createSupportTicket(
    ...
    _isLoading = true;
    notifyListeners();
    http.StreamedResponse response = await supportTicketRepo!
  ...
  Future<void> closeSupportTicket(int? ticketID) async {
    ApiResponse apiResponse =
        await supportTicketRepo!.closeSupportTicket(ticketID.toString());
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      getSupportTicketList();
      showCustomSnackBar(
