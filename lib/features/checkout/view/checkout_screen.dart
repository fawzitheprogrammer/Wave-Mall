...
import 'package:flutter_sixvalley_ecommerce/features/checkout/widget/choose_payment_section.dart';
import 'package:flutter_sixvalley_ecommerce/features/checkout/widget/coupon_apply_widget.dart';
import 'package:flutter_sixvalley_ecommerce/features/checkout/widget/shipping_details_widget.dart';
import 'package:flutter_sixvalley_ecommerce/features/checkout/widget/wallet_payment.dart';
...
class CheckoutScreen extends StatefulWidget {
  ...
  const CheckoutScreen({super.key, required this.cartList, this.fromProductDetails = false,
    required this.discount, required this.tax, required this.totalOrderAmount, required this.shippingFee, this.sellerId, this.onlyDigital = false, required this.quantity, required this.hasPhysical});
...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    ...
    Provider.of<CartController>(context, listen: false).getChosenShippingMethod(context);
    ...
    {
      Provider.of<CheckOutProvider>(context, listen: false).getOfflinePaymentList();
    }
                            ...
                            child: CustomButton(
                              onTap: () async {
                                if(orderProvider.addressIndex == null && widget.hasPhysical) {
                                  ...
                                  locationProvider.billingAddressList[orderProvider.billingAddressIndex!].id.toString() : '';
                                          ...
                                          onTap: (){
                                            if(profile.balance! < (_order + widget.shippingFee - widget.discount - _couponDiscount! + widget.tax)){
                                              ...
                                              orderProvider.placeOrder(callback: _callback,wallet: true,
                                                  addressID : widget.onlyDigital ? '': locationProvider.addressList![orderProvider.addressIndex!].id.toString(),
                                                  couponCode : couponCode,
                                                  couponAmount : couponCodeAmount,
                                                  billingAddressId : _billingAddress? locationProvider.billingAddressList[orderProvider.billingAddressIndex!].id.toString():
          ...
          return Consumer<CheckOutProvider>(
            builder: (context, orderProvider,_) {
              ...
              return Column(children: [
                  ...
                  Expanded(child: ListView(physics: const BouncingScrollPhysics(), padding: const EdgeInsets.all(0), children: [
